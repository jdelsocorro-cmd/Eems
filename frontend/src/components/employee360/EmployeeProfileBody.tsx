...
// The shared body behind BOTH Employee 360's full-page route
// (pages/Employee360.tsx, a thin useParams wrapper around this) and the Org
// Chart's slide-in EmployeeSidePanel -- one implementation, not a fork, so a
// future change to any tab only has to happen once. `compact` is the only
// thing that varies by context: it shrinks the heading and swaps
// PerformanceTab's 3-column table for a stacked list that actually fits a
// ~420px panel (a <table> just crushes or scrolls sideways at that width).
// Read-only workspace, same as Employee 360 always was -- no mutation
// controls anywhere in here. Visibility is entirely RLS's call
// (040_employee_360_hierarchy_visibility.sql's hierarchy_subtree_employee_
// ids, additive to any existing RBAC grant), never re-checked here.
export function EmployeeProfileBody({ employeeId, compact = false }: { employeeId: string; compact?: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <p className={`font-semibold text-text ${compact ? "text-base" : "text-xl"}`}>
      ...
      {activeTab === "performance" && <PerformanceTab kpisQuery={kpisQuery} scoresQuery={scoresQuery} compact={compact} />}
  ...
  const kpisContent = compact ? (
    <Card className="p-4">
      ...
      {kpisQuery.isLoading ? (
        ...
        <EmptyState message="No active KPIs." />
      ) : (
        <ul className="flex flex-col gap-3">
          {activeKpis.map((kpi) => (
            <li key={kpi.id} className="border-b border-border pb-2 last:border-0 last:pb-0">
              <p className="text-sm text-text">
                {kpi.name} <span className="text-xs text-text-dim">({kpi.direction.replace(/_/g, " ")})</span>
              </p>
              <p className="mt-0.5 text-xs text-text-muted">
                {kpi.current_value} / {kpi.target_value} {kpi.unit} · {Math.round(ratio(kpi) * 100)}% · weight {kpi.weight}
              </p>
            </li>
          ))}
        </ul>
